...
SELECT market_date, 
				customer_id,
				DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date ASC) customer_visit_number
...
FROM(
	SELECT DISTINCT market_date, 
					customer_id,
					DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date DESC) customer_visit_number
					
					FROM customer_purchases
)visit_dates

WHERE visit_dates.customer_visit_number = 1
...
CASE
			...
			ELSE  LTRIM(SUBSTR(product_name, INSTR(product_name,'-')+1))
			END AS description
FROM product;
...
DROP TABLE IF EXISTS sum_sales;

CREATE TEMP TABLE sum_sales AS
